...
// ═══════════════════════════════════════════════════════════
// SPELL IT! — Asher & Rivka's Spelling Trainer
// richterfun.com — PRODUCTION BUILD
// localStorage for persistence (replaces Claude's window.storage)
// ═══════════════════════════════════════════════════════════
...
const storage = {
  ...
  set: (key, val) => { try { localStorage.setItem(key, JSON.stringify(val)); } catch (e) { console.log("Storage full", e); } },
};

const FIRE = [
  "LEGEND! 🔥","Benny Friedman approved! 🎤","GESHMAK! ✨","Mamash amazing! 💎",
  "Chanukah menorah energy! 🕎","Ishay Ribo level! 🎶","GADOL move! 🏆",
  "Shabbos cholent vibes! 🍲","Mordechai Shapiro says WOW! 🎵","BOSS! 👑",
  "Fresh challah smooth! 🍞","MaccaBeats remix! 🎧","PRO! 💪","Zusha energy! 🌟",
  "Built like a sukkah! 🏗️","Eight Nights of W's! 🕎","Shepping nachas! ❤️","Tzaddik move! ✡️",
];
const MISS = [
  "Almost! Next one's yours 💪","So close! Even Moshe needed tries 🌟",
  "Not yet — getting stronger ⚡","That's how we learn 📖",
  "Winners keep going 🏆","Close! Get the next one 🎯",
  "Maccabees lost rounds too ⭐",
];

const WORD_DB = {
  double_trouble: {
    name: "Double Trouble", icon: "✌️", color: "#FF6B35", desc: "Tricky double letters",
    words: [
      ...
      {word:"committee",hint:"mm + tt + ee — triple!",sentence:"The ___ voted on new rules.",diff:2},
      ...
      {word:"disappear",hint:"1 s, 2 p's — poof!",sentence:"Watch it ___.",diff:1},
      {word:"balloon",hint:"ll + oo — puffy!",sentence:"The red ___ floated away.",diff:1},
      {word:"broccoli",hint:"cc but only 1 l",sentence:"Eat your ___.",diff:2},
      {word:"embarrass",hint:"rr + ss",sentence:"Don't ___ me!",diff:3},
      {word:"misspell",hint:"ss — ironic!",sentence:"Don't ___ words.",diff:2},
    ],
  },
  ninja_letters: {
    name: "Ninja Letters", icon: "🥷", color: "#7C4DFF", desc: "Silent letters hiding",
    words: [
  ...
  ie_battle: {
    name: "I vs E Showdown", icon: "⚔️", color: "#2979FF", desc: "IE or EI?",
    words: [
      ...
      {word:"weird",hint:"EI — word IS weird",sentence:"That was really ___.",diff:1},
      {word:"their",hint:"EI — exception!",sentence:"___ house is big.",diff:1},
      {word:"ceiling",hint:"EI after C",sentence:"The ___ fan spins.",diff:2},
      {word:"neither",hint:"EI — exception",sentence:"___ answer is right.",diff:2},
  ...
  endings: {
    name: "-TION vs -SION", icon: "🎯", color: "#E91E63", desc: "Which ending?",
    words: [
      {word:"education",hint:"-TION",sentence:"Getting an ___ matters.",diff:1},
      {word:"permission",hint:"-SION (permit→)",sentence:"Ask for ___ first.",diff:2},
      ...
      {word:"decision",hint:"-SION (decide→)",sentence:"A tough ___.",diff:2},
      ...
      {word:"confusion",hint:"-SION (confuse→)",sentence:"Lots of ___.",diff:2},
      {word:"preparation",hint:"-TION",sentence:"___ is key.",diff:2},
      {word:"explosion",hint:"-SION — BOOM!",sentence:"A loud ___.",diff:2},
    ],
  },
  most_wanted: {
    name: "Most Wanted", icon: "🔥", color: "#FF9800", desc: "Everyone gets these wrong",
    words: [
  ...
  able_ible: {
    name: "-ABLE or -IBLE?", icon: "🧩", color: "#00BCD4", desc: "Which ending?",
    words: [
...
const mast=lv=>{if(lv===0)return{l:"New",c:"#78909C",e:"🆕"};if(lv<=2)return{l:"Learning",c:"#FF9800",e:"📖"};if(lv<=4)return{l:"Getting It",c:"#2979FF",e:"💪"};if(lv<=6)return{l:"Almost Pro",c:"#AA00FF",e:"⭐"};return{l:"MASTERED",c:"#00E676",e:"🏆"};};
...
export default function App(){
  ...
  if(!loaded)return(
    <div style={{...PG,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column"}}>
      <style>{GLOBAL_STYLES}</style>
      <link href="https://fonts.googleapis.com/css2?family=Fredoka:wght@300;400;500;600;700&display=swap" rel="stylesheet"/>
      <div style={{fontSize:52,animation:"bn 1s infinite"}}>📝</div>
    </div>
  );
  ...
  if(screen==="pick")return(
    <div style={{...PG,display:"flex",flexDirection:"column",justifyContent:"center",padding:"0 20px"}}>
      ...
      {[
        {id:"asher",name:"Asher",emoji:"🦁",color:"#FF6B35",sub:"The Spelling King"},
        {id:"rivka",name:"Rivka",emoji:"⭐",color:"#AA00FF",sub:"The Word Queen"},
      ].map((u,i)=>{
        const d=data[u.id];
        return(
          <div key={u.id} onClick={()=>{setUser(u.id);setScreen("cats");}}
            ...
            <div style={{textAlign:"right"}}>
              {d.cal.s>0&&<div style={{fontSize:13,color:"#FFD700",fontWeight:600}}>{d.cal.s>=7?"🔥":d.cal.s>=3?"⚡":"✨"} {d.cal.s}</div>}
              {d.lv>1&&<div style={{fontSize:11,color:"rgba(255,255,255,0.3)"}}>Lv {d.lv}</div>}
            </div>
          </div>
        );
      })}
      <div style={{textAlign:"center",marginTop:20,fontSize:11,color:"rgba(255,255,255,0.12)"}}>Built with ❤️ for the Richter family</div>
    </div>
  );
  ...
  if(screen==="cats"){
    ...
    const u=ud;
    const modes=[{id:"type",icon:"🚀",nm:"Spell It"},{id:"unscramble",icon:"🔀",nm:"Unscramble"},{id:"fill",icon:"🕵️",nm:"Fill Gaps"},{id:"speed",icon:"⚡",nm:"Speed"}];
    return(
      <div style={{...PG,padding:"0 0 60px"}}>
        <style>{GLOBAL_STYLES}</style>
        <link href="https://fonts.googleapis.com/css2?family=Fredoka:wght@300;400;500;600;700&display=swap" rel="stylesheet"/>
        <div style={{display:"flex",alignItems:"center",padding:"14px 16px 0",gap:8}}>
          <div onClick={()=>setScreen("pick")} style={{fontSize:13,color:"rgba(255,255,255,0.3)",cursor:"pointer"}}>← Switch</div>
          <div style={{flex:1}}/>
          <div onClick={()=>setScreen("stats")} style={{fontSize:11,color:"rgba(255,255,255,0.2)",cursor:"pointer",padding:"4px 10px",borderRadius:8,background:"rgba(255,255,255,0.04)"}}>📊</div>
        </div>
        <div style={{padding:"10px 16px 4px",display:"flex",alignItems:"center",gap:8}}>
          <div style={{fontSize:22,fontWeight:700}}>Let's go, {nm}!</div>
          <div style={{fontSize:13,color:"#FFD700",fontWeight:600}}>{u.cal.s>=7?"🔥":u.cal.s>=3?"⚡":"✨"}{u.cal.s} · Lv {u.lv}</div>
        </div>
  ...
  if(screen==="stats"){
    ...
    return(
      <div style={{...PG,padding:"0 0 60px"}}>
        <style>{GLOBAL_STYLES}</style>
        <link href="https://fonts.googleapis.com/css2?family=Fredoka:wght@300;400;500;600;700&display=swap" rel="stylesheet"/>
        <div onClick={()=>setScreen("cats")} style={{padding:"14px 16px 0",fontSize:13,color:"rgba(255,255,255,0.3)",cursor:"pointer"}}>← Back</div>
        <div style={{padding:"8px 16px 12px",fontSize:20,fontWeight:700}}>📊 {user==="asher"?"Asher":"Rivka"}'s Stats</div>
        <div style={{display:"flex",gap:8,padding:"0 16px",marginBottom:10}}>
          ...
          <div style={{flex:1,background:"rgba(255,255,255,0.04)",borderRadius:12,padding:"10px",textAlign:"center",border:"1px solid rgba(255,255,255,0.05)"}}>
            <div style={{fontSize:22,fontWeight:700,color:"#FFD700"}}>{u.cal.s>=7?"🔥":"✨"} {u.cal.s}</div>
        ...
        {td&&<div style={{display:"flex",gap:8,padding:"0 16px",marginBottom:10}}>
          {[{v:td.r,l:"Correct",c:"#00E676"},{v:td.r+td.w,l:"Practiced",c:"#2979FF"},{v:td.r+td.w>0?Math.round((td.r/(td.r+td.w))*100)+"%":"—",l:"Accuracy",c:"#FFD700"}].map((s,i)=>(
...
function Sesh({cfg,user,progress,onAnswer,onExit}){
  ...
  if(done){
    ...
    const ln=pct>=90?"ABSOLUTE LEGEND!":pct>=70?"Great session!":pct>=50?"Solid work!":"Keep going — you got this!";
    return(
      <div style={{...PG,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center",padding:24}}>
        ...
        <div onClick={()=>{setIdx(0);setStats({r:0,w:0});setDone(false);setFb(null);setInput("");if(cfg.mode==="speed")setTimer(45);}}
          style={{marginTop:12,fontSize:14,color:ac,cursor:"pointer",fontWeight:600}}>🔄 Again</div>
      </div>
    );
  }
  ...
  return(
    <div style={{...PG,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{display:"flex",alignItems:"center",padding:"14px 16px 8px",gap:10}}>
        <div onClick={onExit} style={{fontSize:14,color:"rgba(255,255,255,0.3)",cursor:"pointer"}}>✕</div>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"0 20px"}}>
        ...
        <div onClick={()=>speak(cw.word,0.7)} style={{fontSize:13,color:ac,cursor:"pointer",marginBottom:10}}>🔊 Hear it</div>
        {(cfg.mode==="type"||cfg.mode==="speed")&&<>
          ...
          <div style={{fontSize:12,color:"rgba(255,255,255,0.2)",marginBottom:14}}>💡 {cw.hint}</div>
        </>}
        {cfg.mode==="unscramble"&&<>
          <div style={{fontSize:12,color:"rgba(255,255,255,0.2)",marginBottom:10}}>💡 {cw.hint}</div>
        ...
        {cfg.mode==="fill"&&gaps&&<>
          <div style={{fontSize:12,color:"rgba(255,255,255,0.2)",marginBottom:12}}>💡 {cw.hint}</div>
      ...
      {(cfg.mode==="type"||cfg.mode==="speed")&&!fb&&(
        <div style={{padding:"12px 16px 20px",background:"rgba(0,0,0,0.2)",borderTop:"1px solid rgba(255,255,255,0.04)"}}>
          <div style={{display:"flex",gap:10}}>
            ...
            <div onClick={sub} style={{padding:"14px 20px",borderRadius:12,background:ac,cursor:"pointer",fontSize:16,fontWeight:700,display:"flex",alignItems:"center"}}>Go ➜</div>
          </div>
        </div>
      )}
      {cfg.mode==="fill"&&!fb&&(
        <div style={{padding:"12px 16px 20px",background:"rgba(0,0,0,0.2)",borderTop:"1px solid rgba(255,255,255,0.04)"}}>
          <div style={{display:"flex",gap:10}}>
            <input value={gapIn} onChange={e=>setGapIn(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")subG();}}
              ...
              style={{flex:1,padding:"14px 16px",borderRadius:12,border:`2px solid ${ac}40`,background:"rgba(255,255,255,0.06)",color:"#fff",fontSize:18,fontFamily:FNT,outline:"none",letterSpacing:1}}/>
            <div onClick={subG} style={{padding:"14px 20px",borderRadius:12,background:ac,cursor:"pointer",fontSize:16,fontWeight:700,display:"flex",alignItems:"center"}}>Go ➜</div>
          </div>
        </div>
      )}
    </div>
